...
| Date | Change |
|------|--------|
| April 2026 | File extension changed from `.mbx.json` to `.mbx`. Verifiers should accept both for backward compatibility with files exported before this change. |
| March 2026 | Initial MBX v2 specification. |
...
MBX files use the `.mbx` extension and are valid JSON. They can be opened in any text editor, parsed by any JSON library, and verified by any implementation of SHA-256.
...
1. **File extension:** `.mbx` (legacy: `.mbx.json`)
...
Read the `.mbx` file and parse it as JSON. If parsing fails, the file is corrupt or not a valid MBX file.
